...
function AppContent() {
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F2] flex flex-col font-body antialiased">
      ...
      {/*
        DEV-ONLY SCREEN SWITCHER — jumps straight to any route by flipping
        local `activeRoute` state. It does NOT go through Firebase auth or
        check profile.status/isAdmin at all, so it must never ship to prod:
        anyone could otherwise click straight into the SuperAdmin dashboard.
        import.meta.env.DEV is replaced/stripped by Vite at build time, so
        this whole block is compiled out of production bundles.
      */}
      {import.meta.env.DEV && (
        <div className="bg-[#1A1A1A] text-white text-xs px-4 py-2 border-b border-white/10 flex flex-wrap items-center justify-between gap-3 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-400 font-mono text-[11px] hidden sm:inline">
              DEV MODE — bypasses Firebase auth
            </span>
          </div>
