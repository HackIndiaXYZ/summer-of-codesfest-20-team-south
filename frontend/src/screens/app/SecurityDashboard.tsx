...
export const SecurityDashboard: React.FC<SecurityDashboardProps> = ({
  ...
  const [visitorPasses, setVisitorPasses] = useState<VisitorPass[]>([
    {
      id: 'VP-1089',
      visitorName: 'Ramesh K',
      visitorPhone: '+91 98401 23456',
      residentName: 'Vaishnavi S',
      room: '204',
      block: 'Vaigai Block A',
      purpose: 'Family Visit & Luggage Drop',
      visitDate: '2026-07-24',
      timeSlot: '02:00 PM - 05:00 PM',
      govtId: 'Aadhaar XXXX 8912',
      status: 'Approved',
    },
    {
      id: 'VP-1090',
      visitorName: 'Kavitha M',
      visitorPhone: '+91 97102 98765',
      residentName: 'Ananya Roy',
      room: '108',
      block: 'Vaigai Block B',
      purpose: 'Books & Laptops Delivery',
      visitDate: '2026-07-24',
      timeSlot: '03:30 PM - 06:00 PM',
      govtId: 'PAN XXXX 415F',
      status: 'Pending',
    },
    {
      id: 'VP-1091',
      visitorName: 'Suresh Babu',
      visitorPhone: '+91 94440 11223',
      residentName: 'Priya Raman',
      room: '302',
      block: 'Vaigai Block A',
      purpose: 'Academic Mentorship',
      visitDate: '2026-07-24',
      timeSlot: '04:00 PM - 06:00 PM',
      govtId: 'Driving License TN-01-XXXX',
      status: 'Pending',
    },
  ]);
  ...
  // Mock SOS
  const [sosAlerts, setSosAlerts] = useState<EmergencySOS[]>([
    {
      id: 'SOS-991',
      studentName: 'Priya R',
      room: '312',
      block: 'Block C',
      emergencyType: 'Medical Emergency - Sudden High Fever & Dizziness',
      timestamp: '12 mins ago',
      priority: 'Critical',
      status: 'Active',
      phone: '+91 98765 11223',
      description: 'Roommates reported high fever, severe weakness. First aid team requested immediately.',
    },
  ]);
  ...
  // HANDLERS
  // -------------------------------------------------------------
  const handleScanPass = (code: string) => {
    ...
    const foundVisitor = visitorPasses.find((v) => v.id.toUpperCase() === query);
    if (foundVisitor) {
      setScannedResultModal({ type: 'visitor', visitorData: foundVisitor });
      showToast({
        title: 'Visitor Pass Verified 🎟️',
        message: `Pass ${foundVisitor.id} loaded successfully for ${foundVisitor.visitorName}`,
        type: 'success',
      });
      return;
    }
    ...
    // Check if Work Pass
    const foundPass = passes.find((p) => p.id.toUpperCase() === query) || passes[0];
    if (foundPass) {
    ...
    showToast({
      ...
      message: `No active pass found matching code "${query}". Verify ID.`,
      type: 'error',
    });
  };

  const handleAllowVisitorEntry = (visitor: VisitorPass) => {
    setVisitorPasses((prev) =>
      prev.map((v) => (v.id === visitor.id ? { ...v, status: 'Checked In' } : v))
    );
    ...
    showToast({
      ...
      message: `Visitor ${visitor.visitorName} allowed entry at Main Gate. Log saved.`,
      type: 'success',
    });
  };
                ...
                <Card className="md:col-span-2 p-6 bg-[#1A1A1A] text-white rounded-[24px] border border-white/10 space-y-5 relative overflow-hidden shadow-2xl">
                  ...
                  <div className="flex items-center justify-between text-xs border-b border-white/10 pb-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => setCameraMode(cameraMode === 'rear' ? 'front' : 'rear')}
                  ...
                  <div className="relative aspect-4/3 rounded-2xl bg-black border-2 border-dashed border-white/20 flex flex-col items-center justify-center overflow-hidden">
                    ...
                    <div className="text-center space-y-3 z-10 p-4 bg-black/40 backdrop-blur-xs rounded-2xl border border-white/10">
                      <QrCode className="w-16 h-16 text-[#996E7D] mx-auto animate-pulse" />
                      <p className="font-mono text-xs text-white/80">
                        Scanning active... hold QR pass steady
                      </p>
                    </div>
                  </div>

                  {/* Quick Demo Pass Trigger */}
                  <div className="p-4 rounded-xl bg-white/5 border border-white/10 space-y-2">
                    <span className="text-[11px] text-white/60 font-mono block">
                      ⚡ Quick Demo Pass Trigger (Click to simulate scan):
                    </span>
                    <div className="flex flex-wrap gap-2">
                      <button
                        onClick={() => handleScanPass('VP-1089')}
                        className="px-3 py-1.5 rounded-lg bg-[#2A5C8A]/40 hover:bg-[#2A5C8A] text-white text-xs font-mono border border-white/20 transition-all"
                      >
                        Scan Visitor Pass (VP-1089)
                      </button>
                      <button
                        onClick={() => handleScanPass(passes[0]?.id || 'WP-9081')}
                        className="px-3 py-1.5 rounded-lg bg-[#2E7D32]/40 hover:bg-[#2E7D32] text-white text-xs font-mono border border-white/20 transition-all"
                      >
                        Scan Maintenance Work Pass ({passes[0]?.id || 'WP-9081'})
                      </button>
                    </div>
                  </div>
                </Card>
          ...
          {activeRoute === '/security/verification' && (
            <div className="space-y-6 animate-fadeIn">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {visitorPasses.map((v) => (
                  <Card key={v.id} className="p-5 space-y-4 border-[#E7E4DF] hover:border-[#2A5C8A] transition-all">
                    <div className="flex items-center justify-between border-b border-[#E7E4DF] pb-3">
                      <div className="flex items-center gap-2">
                        <span className="p-2 rounded-xl bg-[#E3F2FD] text-[#1976D2]">
                        ...
                        <div>
                          ...
                          <span className="font-mono text-xs text-[#996E7D]">{v.id}</span>
                        </div>
                      </div>
                    ...
                    <div className="space-y-2 text-xs bg-[#FAF8F2] p-3 rounded-xl border border-[#E7E4DF]">
                      ...
                      <div className="flex justify-between">
                        <span className="text-[#8E8E93]">Time Slot:</span>
                        <strong className="text-[#1A1A1A]">{v.timeSlot}</strong>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-[#8E8E93]">Purpose:</span>
                        <span className="text-[#1A1A1A] font-medium">{v.purpose}</span>
                      </div>
                      <div className="flex justify-between border-t border-[#E7E4DF] pt-2">
                        <span className="text-[#8E8E93]">Govt ID:</span>
                        <span className="font-mono text-[#2E7D32] font-semibold">{v.govtId}</span>
                      </div>
                    </div>
                ...
                {sosAlerts.map((sos) => (
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#D9534F]/30 pb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="font-body text-xs text-[#666666]">
                            Triggered: <strong className="text-[#D9534F]">{sos.timestamp}</strong> • Phone: {sos.phone}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="p-4 rounded-xl bg-white border border-[#D9534F]/30 text-xs space-y-1">
                      ...
                      <p className="text-[#666666] pt-1">{sos.description}</p>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center justify-end gap-3 pt-2">
                      ...
                      {sos.status === 'Active' ? (
                        ...
                        <span className="text-xs text-[#2E7D32] font-bold flex items-center gap-1">
                          <CheckCircle2 className="w-4 h-4" /> Emergency Resolved
                        </span>
                      )}
                    </div>
                  </Card>
            ...
            {scannedResultModal.type === 'visitor' && scannedResultModal.visitorData && (
              <div className="space-y-4">
                <div className="p-4 rounded-xl bg-[#E3F2FD] border border-[#1976D2]/30 flex items-center justify-between">
                  <div>
                    <h4 className="font-heading text-base font-bold text-[#1976D2]">
                      {scannedResultModal.visitorData.visitorName}
                    </h4>
                    <p className="text-xs text-[#666666]">Phone: {scannedResultModal.visitorData.visitorPhone}</p>
                  </div>
                ...
                <div className="p-4 rounded-2xl bg-[#FAF8F2] border border-[#E7E4DF] space-y-2 text-xs">
                  ...
                  <div className="flex justify-between">
                    <span className="text-[#8E8E93]">Visit Slot:</span>
                    <strong className="text-[#1A1A1A]">{scannedResultModal.visitorData.timeSlot}</strong>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[#8E8E93]">Govt ID Verified:</span>
                    <strong className="text-[#2E7D32] font-mono">{scannedResultModal.visitorData.govtId}</strong>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 pt-2">
                  <Button
                    ...
                    onClick={() => {
                      setScannedResultModal(null);
                      showToast({ title: 'Entry Denied', message: 'Visitor entry rejected.', type: 'error' });
                    }}
                  >
                    Reject Entry
                  </Button>
