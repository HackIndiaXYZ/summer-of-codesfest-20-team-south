...
export const ResidentDashboard: React.FC<ResidentDashboardProps> = ({
  ...
  const [profile, setProfile] = useState({
    name: userName || 'Vaishnavi',
    rollNo: '21CS094',
    email: 'vaishnavi.s@college.edu',
    phone: '+91 98401 98765',
    dob: '2003-08-14',
    block: hostelBlock || 'Vaigai Block',
    floor: 'Floor 2',
    room: roomNumber || '204',
    emergencyName: 'Ramesh Sundaram',
  ...
  const [profileEditForm, setProfileEditForm] = useState({ ...profile });
  ...
  const handleRegisterVisitor = (e: React.FormEvent) => {
    ...
    const newGuest = {
      ...
      date: 'Today, Soon',
      status: 'Pending',
      passCode: `PV-${Math.floor(1000 + Math.random() * 9000)}`,
    };
    ...
    showToast({
      ...
      message: `Guest pass for ${guestName} submitted for gate approval!`,
      type: 'success',
    });
  };
  ...
  const handleTriggerSos = () => {
    ...
    setTimeout(() => {
      showToast({
        ...
        message: 'Security & Wardens notified with live GPS coordinates!',
        type: 'error',
      });
                        ...
                        >
                          <div>
                            ...
                            <span className="block text-[11px] text-[#666666]">
                              {vis.relation} • Pass: <code className="font-mono text-[#2A5C8A]">{vis.passCode}</code>
                            </span>
                          </div>

                          <div className="text-right">
                            <Badge variant={vis.status === 'Approved' ? 'success' : 'warning'} size="sm">
                              {vis.status}
                            </Badge>
                            <span className="block text-[10px] text-[#8E8E93] mt-1">
                              {vis.date}
          ...
          {activeRoute === '/resident/visitors' && (
            <div className="space-y-6 animate-fadeIn">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {visitors.map((vis) => (
                  <Card key={vis.id} className="p-6">
                    <div className="flex items-center justify-between mb-4">
                      <Badge variant={vis.status === 'Approved' ? 'success' : 'warning'} size="sm">
                        {vis.status}
                      </Badge>
                      <span className="text-xs font-mono font-bold text-[#2A5C8A]">{vis.id}</span>
                    </div>
                    ...
                    <p className="font-body text-xs text-[#666666]">
                      Relation: {vis.relation} • Scheduled: {vis.date}
                    </p>

                    <div className="mt-4 p-4 rounded-[16px] bg-[#FAF8F2] border border-[#E7E4DF] text-center">
                      <div className="w-24 h-24 mx-auto bg-white border border-[#E7E4DF] rounded-xl flex items-center justify-center p-2 mb-2 shadow-xs">
          ...
          {activeRoute === '/resident/settings' && (
            <div className="space-y-6 animate-fadeIn max-w-3xl">
              ...
              <Card className="p-6 space-y-4">
                ...
                <div className="flex items-center justify-between pb-3 border-b border-[#E7E4DF]">
                  <div>
                    <h5 className="font-heading text-xs font-bold text-[#1A1A1A]">Emergency Overrides</h5>
                    <p className="font-body text-xs text-[#666666]">Allow loud sound alerts for hostel SOS distress calls</p>
                  </div>
                  <input type="checkbox" defaultChecked className="w-5 h-5 accent-[#996E7D]" />
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white rounded-[24px] max-w-md w-full p-6 text-center shadow-2xl border border-[#D9534F] animate-slideUp">
            ...
            <p className="font-body text-xs text-[#666666] mb-6">
              This will send a high-priority distress signal with your live location (<span className="font-bold">{profile.block}, Room {profile.room}</span>) to Warden & Gate Security.
            </p>
